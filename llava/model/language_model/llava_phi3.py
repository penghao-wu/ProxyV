...
from transformers import Phi3Model, Phi3Config,  Phi3ForCausalLM, Phi3PreTrainedModel
from transformers.models.phi3.modeling_phi3 import Phi3SdpaAttention, Phi3Attention, Phi3FlashAttention2, Phi3DecoderLayer, rotate_half, repeat_kv
	...
	def forward(
		...
		for layer_i, decoder_layer in enumerate(self.layers):
			...
			if not proxyv or layer_i < proxyv_start_layer or not first_forward:
				...
				hidden_states_image_full = self.vision_mlp_layers[layer_i-proxyv_start_layer](hidden_states_image_full, hidden_states_image_proxy.contiguous(), proxyv_reduce_factor, per_crop_token_len, proxyv_attn=proxyv_attn)
...
Phi3DecoderLayer.forward = decoder_forward

def Phi3SdpaAttention_forward(
	self,
	hidden_states,
	kv_states,
	attention_mask = None,
	...
	query_states = self.qkv_proj(hidden_states)[..., :query_pos]
	kv = self.qkv_proj(kv_states)
	key_states = kv[..., query_pos : query_pos + self.num_key_value_heads * self.head_dim]
	value_states = kv[..., query_pos + self.num_key_value_heads * self.head_dim :]
...
def Phi3FlashAttention2_forward(
	self,
	hidden_states,
	kv_states,
	attention_mask = None,
...
Phi3SdpaAttention.forward = Phi3SdpaAttention_forward
Phi3Attention.forward = Phi3Attention_forward
Phi3FlashAttention2.forward = Phi3FlashAttention2_forward
