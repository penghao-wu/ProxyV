    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                from llava.model.language_model.llava_internlm import LlavaInternLMConfig

                lora_cfg_pretrained = LlavaInternLMConfig.from_pretrained(model_path)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaInternLMForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            else:
            ...
            if "mixtral" in model_name.lower():
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaPhiForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif "internlm" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
                cfg_pretrained = AutoConfig.from_pretrained(model_path,trust_remote_code=True)
                model = LlavaInternLMForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            else:
                try:
                    from llava.model.language_model.llava_llama import LlavaConfig
