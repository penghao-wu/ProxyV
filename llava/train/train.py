...
class ModelArguments:
	...
	proxyv: Optional[bool] = field(default=False)
	proxyv_start_layer: Optional[int] = field(default=0)
	learn_proxy: Optional[bool] = field(default=False)
	max_num_image_crops: Optional[int] = field(default=1)
	per_crop_token_len: Optional[int] = field(default=576)
	proxyv_reduce_factor: Optional[int] = field(default=4)
	tune_mm_vision_mlp: Optional[bool] = field(default=False)
	hidden_size_reduce_factor: Optional[int] = field(default=4)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
	...
	if check_only_save_mm_adapter_tunnable:
		# Only save Adapter
		keys_to_match = ["mm_projector", "vision_resampler", 'vision_mlp_layers', 'image_newline', 'proxyv_initializer']
...
def preprocess_internlm2(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
	conv = conversation_lib.default_conversation.copy()
	roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
	...
	if model_args.model_class_name is not None:
		...
		if "mixtral" in model_args.model_name_or_path.lower():
			...
			model = LlavaInternLMForCausalLM.from_pretrained(
				model_args.model_name_or_path,
				cache_dir=training_args.cache_dir,
				attn_implementation=training_args.attn_implementation,
				torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
...
def train(attn_implementation=None):
	...
	if "mistral" in model_args.model_name_or_path.lower() or "mixtral" in model_args.model_name_or_path.lower() or "zephyr" in model_args.model_name_or_path.lower():
		...
		or "llama" in model_args.model_name_or_path.lower()
		or "yi" in model_args.model_name_or_path.lower()
		or "gemma" in model_args.model_name_or_path.lower()
		or "phi" in model_args.model_name_or_path.lower()
		or "nous-hermes" in model_args.model_name_or_path.lower()
		and "wizard-2" in model_args.model_name_or_path.lower()
	):
		...
		if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
			...
			if model_args.tune_mm_vision_mlp:
				for p in model.get_model().vision_mlp_layers.parameters():
					p.requires_grad = True
				if model_args.learn_proxy:
					for p in model.get_model().proxyv_initializer.parameters():
						p.requires_grad = True
			...
			if "mm_language_model" in tunable_parts:
				for name, param in model.named_parameters():
					if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name and "vision_mlp_layers" not in name and "proxyv_initializer" not in name:
						param.requires_grad_(True)
